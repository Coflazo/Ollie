...
418 tests, no network required. A `FakeOllama` test double runs the entire turn pipeline
...
prompts/     OLLIE_SYSTEM.md, the single prompt: contract, voice, all sixteen types, the library
native/      C++20 hot paths, each with a Python twin
web/         React interface
tests/       418 tests, no network required
